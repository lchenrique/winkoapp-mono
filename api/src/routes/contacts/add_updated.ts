...
const addContactSchema = z.object({
  // Accept either contactId (UUID) or username (string)
...
export const addContact: FastifyPluginAsyncZod = async (app) => {
  app.post('/', {
    ...
    schema: {
      tags: ['Contacts'],
      description: 'Add a user to your contacts list by contactId or username',
    ...
    const { contactId, username, nickname } = request.body;
    const userId = (request as any).user.id;

    try {
      // Find contact user by either ID or username
      let contactUser;
      if (contactId) {
